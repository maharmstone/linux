				  ...
				  unsigned int num_entries)
{
	...
	data_sizes = kzalloc(sizeof(u32) * num_entries, GFP_NOFS);
	if (!data_sizes)
		return -ENOMEM;
					   ...
					   u64 addr, s64 diff)
{
	...
	bg = btrfs_lookup_block_group(fs_info, addr);
	bg->remap_bytes += diff;
			       ...
			       u64 length, u64 old_addr)
{
	...
	ret = btrfs_reserve_extent(fs_info->fs_root, length, length,
				   fs_info->sectorsize, 0, 0, &ins,
				   is_data, false);
	if (ret) {
		spin_lock(&sinfo->lock);
		btrfs_space_info_update_bytes_may_use(sinfo, -length);
	...
	adjust_block_group_remap_bytes(trans, new_addr, -dest_length);
	adjust_block_group_remap_bytes(trans, dest_addr, dest_length);
	...
	if (ret) {
		ret = btrfs_free_reserved_extent(fs_info, dest_addr,
						 dest_length, 0);
				     ...
				     struct btrfs_path *path, u64 *last_start)
{
	...
	struct btrfs_block_group *dest_bg;
	struct btrfs_chunk_map *chunk;
	u64 start, remap_length, length, new_addr;
	int ret;
	bool no_more = false;
	bool is_data = src_bg->flags & BTRFS_BLOCK_GROUP_DATA;
	bool made_reservation = false;
	...
	ret = btrfs_reserve_extent(fs_info->fs_root, remap_length,
				   remap_length, fs_info->sectorsize,
	...
	dest_bg->remap_bytes += length;
	btrfs_free_reserved_bytes(dest_bg, length, 0);
	btrfs_put_block_group(dest_bg);

	if (length < remap_length) {
		spin_lock(&sinfo->lock);
		btrfs_space_info_update_bytes_may_use(sinfo,
						      length - remap_length);
		spin_unlock(&sinfo->lock);
	}
next:
					...
					u64 *bytenr, u64 *num_bytes)
{
	...
	if (*bytenr == remap_start && *num_bytes >= remap_length) {
		...
		if (key.type != BTRFS_IDENTITY_REMAP_KEY) {
			...
			adjust_block_group_remap_bytes(trans, new_addr,
						       -remap_length);
		} else {
